...
import { fetchReservationCheckInOutInServer } from '@/services/reservationService'
...
export default async function SpaceCheckInOutPage(props: { params: Params }) {
  ...
  const checkInOut = await fetchReservationCheckInOutInServer(space.id)
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col items-center justify-center gap-4">
        ...
        >{`Sala ${space.name}`}</Text>

        {renderCheckInOutFeedback(checkInOut?.message)}
      </div>
    </div>
  )
}
