  ...
  return (
    ...
    >
      ...
      <div className="mt-5 grid w-full max-w-6xl grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        {/* Card Postos Avançados - visível para admin, dev, director e supervisor */}
        {(user?.role === 'admin' ||
          user?.role === 'dev' ||
          teamPosition === 'director' ||
          teamPosition === 'supervisor') && (
          <Link
            href="/equipe/postos-avancados"
            className="flex flex-col items-center gap-2.5 rounded-lg bg-cyan-600 p-6 shadow-xl transition-colors hover:bg-cyan-700"
          >
            <div className="relative flex items-center justify-center">
              <CardDecoration className="absolute bottom-[-15px] left-[-15px] text-white/20" />
              <Building2 size={48} className="z-10 text-white" />
            </div>
            <Text
              variant="title-18-24-700"
              className="mt-4 text-center text-white"
