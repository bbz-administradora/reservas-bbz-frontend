...
export async function fetchAvailableRoomSlotsByRoom(
  roomId: string,
  ...
  const getGetRoomSlotAvailabilityUrl = (
    roomId: string,
    params: GetRoomSlotAvailabilityParams,
  ) => {
    const normalizedParams = new URLSearchParams()
  ...
  const url = getGetRoomSlotAvailabilityUrl(roomId, params)

  const response = await customFetch<GetRoomSlotAvailability200>(url, {
    method: 'GET',
    credentials: 'include',
    cache: 'no-store',
    headers,
